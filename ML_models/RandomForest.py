...
for LLM in LLMs:
    ...
    # DataFrames to accumulate classified cases
    ...
    for i, state in enumerate(random_states):
        ...
        os.makedirs(f"ML_models/feature_importance/models/RandomForest_{LLM}", exist_ok=True)
        os.makedirs(f"ML_models/feature_importance/results/models/RandomForest_{LLM}", exist_ok=True)
        
        # Save the model and vectorizer
        joblib.dump(model, f"ML_models/feature_importance/models/RandomForest_{LLM}/model_{i+1}.joblib")
        joblib.dump(vectorizer, f"ML_models/feature_importance/models/RandomForest_{LLM}/vectorizer_{i+1}.joblib")
    ...
    confusion_df.to_csv(f"ML_models/results/RandomForest_{LLM}/confusion_matrices.csv", index=False)

    # Save accumulated misclassifications
    misclassified_llm_all.to_csv(f"ML_models/results/RandomForest_{LLM}/misclassified_LLM_all.csv", index=False)
    misclassified_student_all.to_csv(f"ML_models/results/RandomForest_{LLM}/misclassified_Student_all.csv", index=False)
    
    # Save accumulated classified cases
    classified_llm_all.to_csv(f"ML_models/results/RandomForest_{LLM}/classified_LLM_all.csv", index=False)
    classified_student_all.to_csv(f"ML_models/results/RandomForest_{LLM}/classified_Student_all.csv", index=False)
    ...
    graph.render(f"ML_models/results/RandomForest_{LLM}/tree_visualization")
